...
__ExportExecutorSubdir="admin/core/extensions/exportstaging/java/dist"

_CS_WEBROOT=${__ScriptDir%$__ExportExecutorSubdir}
_CS_PROJECTS=""
...
_CS_ACTIVEMQ_COMMAND="sh ${_CS_WEBROOT}/admin.local/lib/activemq/bin/activemq start"
_CS_ACTIVEMQ_PID="${_CS_WEBROOT}/admin.local/lib/activemq/data/activemq.pid"
...
_CS_ELASTIC_COMMAND="sh ${_CS_WEBROOT}/admin.local/lib/elasticsearch/bin/elasticsearch -d"
...
_CS_EE_JAVAOPTS="-Xms2048M -Xmx8192M"
...
__start_cassandra() {
    ...
    then
        ...
        ps ax | grep java | grep "org.apache.cassandra" | grep -v "grep" | awk '{print $1}' > $_CS_CASSANDRA_PID
...
__start_elasticsearch() {
    ...
    then
        ...
        ps ax | grep java | grep "org.elasticsearch" | grep -v "grep" | awk '{print $1}' > $_CS_ELASTIC_PID
    ...
    while ! (curl -s http://${_CS_ELASTIC_HOST}:${_CS_ELASTIC_PORT}/_cluster/health 2>>/dev/null | grep -q "\"status\":\"green\"") ;
    do
        HC_COUNTER=$((HC_COUNTER-1))
        if [ "$HC_COUNTER" -le 0 ];
        then
...
__start_all_projects() {
    ...
    do
        ...
        if curl -s http://${_CS_PROJECT_ELASTIC_HOST}:${_CS_PROJECT_ELASTIC_PORT}/_cluster/health | grep -q "\"status\":\"green\"";
        then
            echoinfo "Successfully connected"
        else
            echowarn "Unable to connect, skipping project $_CS_PROJECT"
        ...
        then
            ...
            _CS_EE_LOG="${_CS_WEBROOT}/${_CS_PROJECT}/data/exportstaging/cs-ee.log"
            _CS_EE_PID="${_CS_WEBROOT}/${_CS_PROJECT}/data/exportstaging/cs-ee.pid"
            if (sudo -u $_CS_USER -H nohup $_CS_EE_COMMAND $_CS_PROJECT >>$_CS_EE_LOG 2>>$_CS_EE_LOG &) ;
            then
                sleep $__DEFAULT_SLEEP
                echodebug "Changing the file owner to ${_CS_USER}:${_CS_GROUP}"
                chown ${_CS_USER}:${_CS_GROUP} $_CS_EE_LOG
